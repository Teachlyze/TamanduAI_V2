...
export const ClassService = {
  ...
  async getClassStats(classId) {
    try {
      ...
      return {
        ...
        pendingCorrections: pendingSubmissions?.length || 0,
  ...
  /**
   * Generate new invite code for class
   * @param {string} classId - The ID of the class
  ...
  /**
   * Archive a class
   * @param {string} classId - The ID of the class to archive
   * @returns {Promise<boolean>} - True if successful
   */
   ...
   * @param {string} classId - The ID of the class
   * @param {Object} options - Query options
   * @param {string} [options.role] - Filter by role (teacher/student)
   * @returns {Promise<Array>} - Array of members with profiles
   */
  async getClassMembers(classId, { role = null } = {}) {
    let query = supabase
      .from('class_members')
      .select(`
        *,
