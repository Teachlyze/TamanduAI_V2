...
serve(async (req) => {
  ...
  try {
    const supabaseClient = createClient(Deno.env.get('SUPABASE_URL') ?? '', (Deno.env.get("SUPABASE_SECRET_KEY") || (Deno.env.get("SUPABASE_SECRET_KEY") || Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")))?? ''
    );
